...
function LumiAnimSeqLoadAndCrop:onUpdate(comp, deltaTime)
    ...
    elseif self.animSeqType == "Texture" then
        ...
        if self.seqTex then
            ...
            self.material:setTex("u_seqTexture", self.InputTex)
            local inputWidth = Amaz.BuiltinObject.getInputTextureWidth()
            local inputHeight = Amaz.BuiltinObject.getInputTextureHeight()
            local videoSize = Amaz.Vector2f(inputWidth, inputHeight)
            if self.enableVideoAlphaBlend then
                videoSize.x = videoSize.x / 2
            end
            self.material:setVec2("u_seqTexSize", videoSize)

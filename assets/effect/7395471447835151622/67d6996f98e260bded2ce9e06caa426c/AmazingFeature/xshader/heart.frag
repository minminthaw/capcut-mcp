...
//    float time = clamp(timer,0.0,1.0);
	...
	vec2 centerUV = vec2(0.5)*ratio;
    vec2 realUV = uv*ratio;
	...
	if(time<0.3)
	{
		zoomCoeff = 1.-0.1*(1.-abs(0.3-time)/0.06);
	...
	float nowBlur = clamp((time-0.24)/0.4,0.,1.);
    ...
    for(float i=1.0;i<=MAX_VALUE;i+=1.0)
    {
        ...
        vec2 curRightCoordinate = (realUV+curStep*nowBlur)/ratio;//textureCoordinate+curStep;
        vec2 curLeftCoordinate  = (realUV-curStep*nowBlur)/ratio;//textureCoordinate-curStep;

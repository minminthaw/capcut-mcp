...
local XiaoQiaoBi = XiaoQiaoBi or {}
XiaoQiaoBi.__index = XiaoQiaoBi

local FACE_ADJUST = "face_adjust_XiaoQiaoBi"
local FACE_ID = "id"
local FACE_ADJUST_INTENSITY = "intensity"
local RESET_PARAMS = "reset_params"

local default_weight = 0.9
...
function XiaoQiaoBi.new(construct, ...)
    local self = setmetatable({}, XiaoQiaoBi)
...
function XiaoQiaoBi:onStart(comp)
...
function XiaoQiaoBi:onUpdate(comp, deltaTime)
...
function XiaoQiaoBi:onEvent(comp, event)
    if event.type == Amaz.AppEventType.SetEffectIntensity then
        ...
        elseif inputKey == FACE_ADJUST then
            ...
            for i = 0, inputSize - 1 do
                ...
                self.faceAdjustMaps[inputId] = inputIntensity  * 0.7  -- we want the slider show 100, but 70 as the actual value used
            end
        end
    end
end

function XiaoQiaoBi:updateFaceInfoBySize()
    self.faceInfoBySize = {}

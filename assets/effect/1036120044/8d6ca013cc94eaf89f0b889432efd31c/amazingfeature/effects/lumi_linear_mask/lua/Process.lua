...
function Process:onStart(comp)
    ...
    self.out_cam = comp.entity:searchEntity("linearCamera"):getComponent("Camera")
end
...
function Process:setEffectAttr(key, value, comp)
    ...
    elseif key == "rotation" or key == "rotationZ" then
        local rotate = -value * 2.0 * math.pi / 360.0
        local orient = Amaz.Vector2f(-math.sin(rotate), math.cos(rotate))

...
function Stack:pop()
    if self:isEmpty() then
        Amaz.LOGW(AE_EFFECT_TAG, "stack is nil")
    end
...
function SeekModeScript:onStart(comp, sys)
    ...
    Amaz.LOGS(self.logTag, "SeekModeScript:onStart() onStart finished")

end

function SeekModeScript:onUpdate(sys, deltaTime)
    
    -- Amaz.LOGS(self.logTag, "SeekModeScript:onUpdate() ")
    ...
    if self.first_frame then
        if #self.faceInfoBySize == 0 then 
            Amaz.LOGW(self.logTag, "SeekModeScript:onUpdate() no face data, skip")
            return
        end
    ...
    local facefittingCount = result:getAlgorithmInfoCount('Editor_Sticker_Config_TAG_xbtpH2d', 'facefitting_3d_0', 'facefitting_3d') 
    ...
    for i = 0, self.FaceCount - 1 do
        ...
        if face.track_id >= 0 then
            ...
            if (faceInfo == nil) then
                Amaz.LOGW(self.logTag, "SeekModeScript:onUpdate() expired face detected, track_id:" .. tostring(face.track_id))
                face.entity.visible = false
                face.size = 0
                face.track_id = -1
    ...
    for i = 1, #self.faceInfoBySize do
        ...
        if (face == nil) then
            Amaz.LOGS(self.logTag, "SeekModeScript:onUpdate() new face detected ")
            ...
            if (face ~= nil) then
                ...
                Amaz.LOGS(self.logTag, "SeekModeScript:onUpdate() set up new face data")
    ...
    for i = 1, #self.faceInfoBySize do
        ...
        if face ~= nil then
            ...
            local faceMeshInfo = result:getAlgorithmInfo('Editor_Sticker_Config_TAG_xbtpH2d', 'facefitting_3d_0', 'facefitting_3d', faceInfo.index) 
            if (not faceMeshInfo) then
                Amaz.LOGS(self.logTag, "SeekModeScript:onUpdate() algorithm result of id " .. i .. " is empty.")
            else
...
function SeekModeScript:updateBrushCanvas()
    ...
    if self.canvasSize ~= nil then
        ...
        -- Amaz.LOGS(self.logTag, "onUpdate() input Size:".. tostring(inputTex.width).. " ".. tostring(inputTex.height))
        -- Amaz.LOGS(self.logTag, "onUpdate() canvasSize:".. tostring(self.canvasSize.width).. " ".. tostring(self.canvasSize.height))
        -- Amaz.LOGS(self.logTag, "onUpdate() resolution Size:".. tostring(resolution.x).. " ".. tostring(resolution.y))
        maxScale = math.min(self.canvasSize.width/resolution.x, self.canvasSize.height/resolution.y)
        -- Amaz.LOGS(self.logTag, "onUpdate() maxScale:".. tostring(maxScale))
    end
...
function SeekModeScript:loadManualRetouchCache()
    ...
    local content = nil
    self.originalTexturePath = {}
    Amaz.LOGS(self.logTag, "SeekModeScript:loadManualRetouchCache() ".. file_path)
    if self:fileExist(file_path) then
        content = self:ReadFromJson(file_path)
    else
        Amaz.LOGW(self.logTag, "SeekModeScript: retouch_config.json not found.")
        return
    end
    
    self.config_data = cjson.decode(content)
    local currentList = self.config_data[self:getBeautyListType()]
    Amaz.LOGD(self.logTag, "SeekModeScript:loadManualRetouchCache() currentList ".. cjson.encode(currentList))
    if currentList == nil then
        Amaz.LOGW(self.logTag, "SeekModeScript:loadManualRetouchCache() currentList is empty.")
        return
    end
    
    for track_id, imageName in pairs(currentList) do
        Amaz.LOGD(self.logTag, "SeekModeScript:loadManualRetouchCache() track_id ".. track_id.. " imageName ".. imageName)
...
function SeekModeScript:onTouchEvent(sys, event)
    ...
    -- Amaz.LOGS(self.logTag, "SeekModeScript: touchEvent:  ".. point.x.. " ".. point.y)
    ...
    if isTouchBegin then
        ...
        if self.selectedFaceInfo ~= nil then
            Amaz.LOGS(self.logTag, "SeekModeScript: selectedFace   ".. cjson.encode(self.selectedFaceInfo))
            self.selectedEntityIndex = 1
        end
    end
...
function SeekModeScript:updateProperties()
    ...
    -- Amaz.LOGD(self.logTag, "SeekModeScript: updateProperties() config.json ".. cjson.encode(content))
    ...
    -- Amaz.LOGD(self.logTag, "SeekModeScript: updateProperties() brush_state ".. cjson.encode(brush_state))
...
function SeekModeScript:handleIntensityEvent(sys, event)
    ...
    -- Amaz.LOGS(self.logTag, "handleEvent key: " .. tostring(inputKey) .. " value: " .. tostring(inputValue))
...
function SeekModeScript:undo()
    if self.undoStack:size() <= 0 then
        Amaz.LOGW(self.logTag, "SeekModeScript:undo() undoStack is empty.")
        return
    end
...
function SeekModeScript:redo()
    if self.redoStack:size() <= 0 then
        Amaz.LOGW(self.logTag, "SeekModeScript:redo() redoStack is empty.")
        return
    end
...
function SeekModeScript:findLastImageByTrackId(track_id)
    ...
    for item in self.undoStack:iterate()() do
        -- Amaz.LOGD(AE_EFFECT_TAG, "item: " .. item.track_id .. " mask_file: ".. item.mask_file)
...
function SeekModeScript:loadExternalTexture(face)
    ...
    -- Amaz.LOGD(self.logTag, "SeekModeScript:loadExternalTexture() track_id ".. tostring(track_id))
    local targetImage = self:findLastImageByTrackId(track_id)
    face.externalFaceTexture = nil
    -- Amaz.LOGD(self.logTag, "SeekModeScript:loadExternalTexture() set externalFaceTexture  nil. ")
    if targetImage ~= nil then
        ...
        if self:fileExist(mask_path) then
            ...
            Amaz.LOGS(self.logTag, "SeekModeScript: loadExternalTexture() load texture: ".. mask_path)
            return targetImage     
        end
    end
    
    if face.externalFaceTexture == nil then
        -- Amaz.LOGD(self.logTag, "SeekModeScript:loadExternalTexture() try to load origin ." .. cjson.encode(self.originalTexturePath))
        if self.originalTexturePath ~= nil and self.originalTexturePath[tostring(track_id)] ~= nil and self:fileExist(self.draft_path.. self.originalTexturePath[tostring(track_id)]) then
            ...
            -- Amaz.LOGD(self.logTag, "SeekModeScript: loadExternalTexture() load origin: ".. fullPath .. " cost: ".. elapsed_time.." ms")
            return self.originalTexturePath[tostring(track_id)]
        else
            face.externalFaceTexture = self.emptyTexture
            -- Amaz.LOGD(self.logTag, "SeekModeScript: loadExternalTexture() load empty texture.")
        end
    end
    -- end
   return nil
...
function SeekModeScript:getBeautyListType()
    ...
    elseif self.brush_type == "spot" then
...
function SeekModeScript:cmdBufExec1(sys, camera, eventType)
    ...
    if self.selectedFaceInfo == nil then
        Amaz.LOGS(self.logTag, "SeekModeScript:cmdBufExec1()  not selected skip render camera1.")
    ...
    if (face.entity.visible and material ~= nil) then
        Amaz.LOGD(self.logTag, "SeekModeScript:cmdBufExec1() start render face")
        ...
        if externalFaceTexture ~= nil then
                -- Amaz.LOGD(self.logTag, "SeekModeScript:cmdBufExec1() use external texture")
            material:setInt("uUseRT",0)
            material:setTex("uHistoryStrokeTexure", externalFaceTexture)
        else
            if historyFaceTexture == nil then
                -- Amaz.LOGD(self.logTag, "SeekModeScript:cmdBufExec1() use empty texture in first frame")
                material:setTex("uHistoryStrokeTexure",self.emptyTexture)
                material:setInt("uUseRT",0)
            else
                -- Amaz.LOGD(self.logTag, "SeekModeScript:cmdBufExec1() use history texture")
...
function SeekModeScript:cmdBufAfterExec1(sys, camera, eventType)
    ...
    local face = self:findFaceByTrackId(self.selectedFaceInfo.track_id)
    -- Amaz.LOGD(self.logTag, "SeekModeScript:cmdBufAfterExec1() track_id: ".. tostring(face.track_id) .. " faces count " .. tostring(#self.faces))
    ...
    if self.draft_path ~= "" then
        ...
        Amaz.LOGS(self.logTag, "SeekModeScript:cmdBufAfterExec1() will save texture to ".. self.nextFrameSavePath)
    end
    ...
    -- Amaz.LOGD(self.logTag, "SeekModeScript:cmdBufAfterExec1() insert opration to config_data")
    self:updateProperties()
...
function SeekModeScript:updateTexture(face)
    ...
    if material ~= nil then
        ...
        local highPassRT = table:get("RT5")
        local lowFqRT = table:get("RT2")
        local faceExtraMaskRT = table:get("FaceExtraMaskRT")
        ...
        material:setTex("u_highPassTex", highPassRT)
        material:setTex("u_faceExtraMaskTex", faceExtraMaskRT)
        material:setTex("u_blurTex", lowFqRT)
        material:setFloat("u_faceFactor", faceFactor)
    end
end

function SeekModeScript:cmdBufAfterExec2(sys, camera, eventType)
    -- Amaz.LOGW(self.logTag, "SeekModeScript:cmdBufAfterExec2()")
...
function SeekModeScript:saveTextureNextFrame()
    if self.nextFrameSaveTexture ~= nil and self.nextFrameSavePath ~= "" then
        if self.nextFrameSaveTexture.saveToFile ~= nil then
            ...
            Amaz.LOGD(self.logTag, "SeekModeScript:saveTextureNextFrame()   " .. self.nextFrameSavePath .. " cost: ".. tostring(elapsed_time) .. " ms")
            self.nextFrameSaveTexture = nil
            self.nextFrameSavePath = ""
        else
            Amaz.LOGW(self.logTag, "SeekModeScript: saveToFile is nil")
        end
    end
end
...
function SeekModeScript:findTouchBeginFaceInfo(point)
    ...
    -- Amaz.LOGS(self.logTag, "self.faceInfoBySize size".. #self.faceInfoBySize)
    for i = 1, #self.faceInfoBySize do
        local faceInfo = self.faceInfoBySize[i]
        ...
        if id ~= -1 then
            ...
            -- Amaz.LOGS(self.logTag, "point.x: ".. point.x.. " point.y: ".. point.y .. "min_x: ".. min_x.. " min_y: ".. min_y.. " max_x: ".. max_x.. " max_y: ".. max_y)
...
function SeekModeScript:updateFaceInfoBySize()
    ...
    -- Amaz.LOGD(self.logTag, "updateFaceInfoBySize faceInfoBySize size "..(#self.faceInfoBySize))
end

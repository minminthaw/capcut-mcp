...
void main()
{
    ...
    for (float _t7 = 1.0; _t7 <= _157; _t7 += mix(1.0, 2.0, step(24.0, blurSize)))
    {
        ...
        _t3 = _180;
        vec2 param_4 = v_uv + vec2(_t7 * _t2.x, 0.0);
        _t5 += ((texture2D(inputImageTexture, v_uv + vec2(_t7 * _t2.x, 0.0)) * _180) * _f0(param_4));
        vec2 param_5 = v_uv - vec2(_t7 * _t2.x, 0.0);
        _t5 += ((texture2D(inputImageTexture, v_uv - vec2(_t7 * _t2.x, 0.0)) * _t3) * _f0(param_5));
        _t4 += (2.0 * _t3);
    }
    vec4 _238 = _t5;

...
fragment main0_out main0(main0_in in [[stage_in]], constant buffer_t& buffer, texture2d<float> inputImageTexture [[texture(0)]], sampler inputImageTextureSmplr [[sampler(0)]])
{
    ...
    for (float _t7 = 1.0; _t7 <= _157; _t7 += mix(1.0, 2.0, step(24.0, buffer.blurSize)))
    {
        ...
        _t3 = _180;
        float2 param_4 = in.v_uv + float2(0.0, _t7 * _t2.y);
        _t5 += ((inputImageTexture.sample(inputImageTextureSmplr, (in.v_uv + float2(0.0, _t7 * _t2.y))) * _180) * _f0(param_4));
        float2 param_5 = in.v_uv - float2(0.0, _t7 * _t2.y);
        _t5 += ((inputImageTexture.sample(inputImageTextureSmplr, (in.v_uv - float2(0.0, _t7 * _t2.y))) * _t3) * _f0(param_5));
        _t4 += (2.0 * _t3);
    }
    float4 _238 = _t5;

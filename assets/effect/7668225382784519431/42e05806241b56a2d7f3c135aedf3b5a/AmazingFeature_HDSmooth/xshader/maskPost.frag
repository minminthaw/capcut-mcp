...
void main() { 
    ...
    lowp float skinColorAlpha = texture2D(skinsegMaskTexture, vec2(textureCoordinate.x, 1.0 - textureCoordinate.y)).a;
    ...
    if (useFaceMask > 0) {
        ...
        lowp vec3 faceMask = texture2D(faceMaskTexture, textureCoordinate).rgb;

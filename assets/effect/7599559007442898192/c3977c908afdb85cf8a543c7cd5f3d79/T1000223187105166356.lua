...
local function funcEaseBlurAction3(t, b, c, d)
	t = t / d
	t = math.abs(math.pow(2, -5.0 * t) * math.log(2) * math.sin(2.5 * math.pi * t - 0.5 * math.pi) +
				...
				if type(func) == 'function' then
					...
					elseif type(keyframe[3]) == 'table'
						...
						for i = 1, #keyframe[3] do
							values[i] = func(t, keyframe[3][i], keyframe[4][i] - keyframe[3][i],
...
local function loadAsset(self, path)
	local asset = nil
	asset = self.comp.assetMgr:SyncLoad(path)
	return asset
end
...
local function anchor(pivot, anchor, halfWidth, halfHeight, translate, rotate, scale)
	...
	mat:setTRS(
		...
		Amaz.Quaternionf.eulerToQuaternion(Amaz.Vector3f(rotate.x / 180 * math.pi, rotate.y / 180 * math.pi,
			rotate.z / 180 * math.pi)),
		Amaz.Vector3f(1, 1, 1)
	)
...
local T1000223187105166356 = T1000223187105166356 or {}
T1000223187105166356.__index = T1000223187105166356
---@class T1000223187105166356 : ScriptComponent
---@field sharedMaterial Material

function T1000223187105166356.new(construct, ...)
	local self = setmetatable({}, T1000223187105166356)
	self.duration = 1.0
	self.count = 0
	self.sharedMaterial = nil
	if construct and T1000223187105166356.constructor then T1000223187105166356.constructor(self, ...) end
	return self
end

function T1000223187105166356:constructor()

end

function T1000223187105166356:onStart(comp)
	self.comp = comp
	...
	self.attrs = AETools.new(AEData.ae_attribute)
	-- self.animVideoCom0 = comp.entity:getComponent("AnimSeqComponent")
	-- if self.animVideoCom0 == nil then
	-- 	Amaz.LOGI("self.mp4" ,111111)
	-- else
	-- 	Amaz.LOGI("self.mp4" ,222222)

	-- end
	-- Amaz.LOGI("222222" ,tostring(self.mp4.VideoFilepath))
...
function T1000223187105166356:animate()
	return {
		-- ['playSpeed'] = 30,
		["anchor"] = { 0, 0 },
		["pivot"] = { 0, 0 },
...
function T1000223187105166356:animateChar(char)
	return {
		['mode'] = 0,
		['duration'] = .8,
		['anchor'] = { 0, 0 },
...
local time = 0
function T1000223187105166356:onUpdate(comp, deltaTime)
	if isEditor then
		time = time + deltaTime
...
function T1000223187105166356:seek(time)
	self.sequenceFrames = {}
	for i = 1, 28 do -- Adjust the number based on your actual frame count
		local framePath = string.format("seq/gilitch-di/gilitch-di_%05d.png", i - 1)
		table.insert(self.sequenceFrames, loadAsset(self, framePath))
	end
	-- local seqTex = nil
	local realTime = time
	local frameIndex = math.floor(realTime * 30) + 1
	frameIndex = frameIndex % #self.sequenceFrames
	if frameIndex == 0 then
		frameIndex = #self.sequenceFrames
	end

	local seqTex = self.sequenceFrames[frameIndex]

	if self.first then
		local materials = Amaz.Vector()
		self.ori_materials = self.renderer.materials
	...
	-- self.animVideoCom0.enableFixedSeekMode = true
	local offsetxValue = 0
	local offsetxResult = self.attrs:GetVal("offsetx", time)
	if offsetxResult and #offsetxResult > 0 then
		offsetxValue = offsetxResult[1] / 1
	end
	if seqTex and self.materials then
		self.materials:get(0):setFloat('u_offsetx', (offsetxValue-0.5)*0.05)
		self.materials:get(0):setTex('_MainTex2', seqTex)
	end
	-- self.animVideoCom0:seekToTime(time)
	...
	if self.text ~= nil then
		...
		for i = 1, self.count do
			...
			translate, rotate, scale = anchor(info['pivot'], info['anchor'], char.width / 3, char.height / 3, translate,
				rotate, scale)
	...
	playAnimation(info, (time / self.duration) % 1, function(key, value)
		if key == 'translate.x' then
			translate.x = value
		elseif key == 'translate.y' then
			translate.y = value
		...
		elseif key == 'percent' then
			self.materials:get(0):setFloat('u_percent', value)
		elseif key == 'transition' then
			self.materials:get(0):setFloat('u_transition', value)
		end
	end)
...
function T1000223187105166356:setDuration(duration)
	self.duration = duration
end

function T1000223187105166356:onEnter()

end

function T1000223187105166356:onLeave()
	self.renderer.materials = self.ori_materials
end

function T1000223187105166356:clear()
	--if self.tag ~= "T1000223187105166356" then
	--	return true
	--end
	if self.text ~= nil then
	...
	self.renderer.materials = self.ori_materials
	self.materials:get(0):setFloat('u_percent', 0.0)
	self.materials:get(0):setFloat('u_transition', 0.2)
end

exports.T1000223187105166356 = T1000223187105166356
return exports

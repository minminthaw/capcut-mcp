...
function LumiManager:onUpdate(comp, deltaTime)
    ...
    if endTime < curTime then
        ...
        elseif self.animationMode == 2 then
            aeTime = self.compDurations[1] + (curTime - startTime) / lvDuration * aeDuration
        elseif self.animationMode == 3 then
            aeTime = self.compDurations[1] + (curTime - startTime) / lvDuration * aeDuration % aeDuration
        end
    end
    ...
    if self.initialized == false then
        self.LumiAnimSeqLoadAndCrop_110 = Amaz.ScriptUtils.getLuaObj(comp.entity.scene:findEntityBy("LumiAnimSeqLoadAndCrop_110-effect0"):getComponent("ScriptComponent"):getScript())
        self.LumiAnimSeqLoadAndCrop_115 = Amaz.ScriptUtils.getLuaObj(comp.entity.scene:findEntityBy("LumiAnimSeqLoadAndCrop_115-effect0"):getComponent("ScriptComponent"):getScript())
        self.LumiSGlow_102 = Amaz.ScriptUtils.getLuaObj(comp.entity.scene:findEntityBy("LumiSGlow_102-effect0"):getComponent("ScriptComponent"):getScript())
        self.LumiSGlow_96 = Amaz.ScriptUtils.getLuaObj(comp.entity.scene:findEntityBy("LumiSGlow_96-effect0"):getComponent("ScriptComponent"):getScript())

        self.initialized = true
    end

    self.LumiAnimSeqLoadAndCrop_110.opacity = self.filterVal
    self.LumiAnimSeqLoadAndCrop_115.opacity = self.filterVal
    self.LumiSGlow_102.brightness = self.filterVal * 0.7
    self.LumiSGlow_96.brightness = self.filterVal * 0.7

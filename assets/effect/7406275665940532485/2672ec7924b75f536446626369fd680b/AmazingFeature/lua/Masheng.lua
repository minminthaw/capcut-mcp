...
local Masheng = Masheng or {}
Masheng.__index = Masheng

local FACE_ADJUST = "face_adjust_MaShengNose"
local FACE_ID = "id"
local FACE_ADJUST_INTENSITY = "intensity"
local RESET_PARAMS = "reset_params"

local default_weight = 0.62
...
function Masheng.new(construct, ...)
    local self = setmetatable({}, Masheng)
...
function Masheng:onUpdate(comp, deltaTime)
...
function Masheng:onEvent(comp, event)
...
function Masheng:updateFaceInfoBySize()
    self.faceInfoBySize = {}

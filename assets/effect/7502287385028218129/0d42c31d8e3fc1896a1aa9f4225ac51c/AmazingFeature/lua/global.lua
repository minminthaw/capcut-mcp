...
local GRAPH_NAME = "05d5e017-bcff-11eb-a04a-acde480011220"
...
function global:onUpdate(sys, deltaTime)
    ...
    for i = 1, math.min(faceCount, self.maxFaceNum, self.groupNum) do
        local intensity = 0
        local groupTexNum = self.groupTexNumList[i]
        ...
        if math.abs(intensity) > INTENSITY_EPSC then
            local ganTex = self:getGanTexture(result, self.graph_name, "script_0",
                index * groupTexNum + self.globalTexNum)
            local tfmInfo = result:getNHImageTfmInfo(self.graph_name, "FaceAlign", index)

            if ganTex and tfmInfo then

...
---@field intensity number [UI(Range={0., 1000.}, Drag)]
...
function ScriptCompGaussianBlur:onUpdate(comp, deltaTime)
    ...
    self.matGaussianBlurY:setFloat("u_opacity", self.opacity)
    ...
    local intensityX = self.intensity * xStrenth
    local intensityY = self.intensity * yStrenth
    local sampleNum, downScale = getSampleNum(self.intensity)

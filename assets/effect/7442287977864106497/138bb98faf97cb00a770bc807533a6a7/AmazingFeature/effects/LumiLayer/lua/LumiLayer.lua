...
---@field opacityMix boolean
---@field transform_bool boolean
---@field transform_bool2 boolean
---@field yuCheng boolean
---@field yuChengFloat number
---@field yuChengFloat2 number
---@field ImageBig boolean
...
function LumiLayer.new(construct, ...)
    ...
    self.transform_bool = false
    self.transform_bool2 = false
    self.opacityMix = false
    self.yuCheng = false

    self.ImageBig = false
    self.yuChengFloat =1
    self.yuChengFloat2 =1
    -- for test
    if false then
        self.aeTime = 1
...
function LumiLayer:updateTRS(ratio)
    ...
    if self.transform_bool then
        if self.InputTex.width >= self.InputTex.height then
            -- posY=posY-350
    ...
    self.father_trans.localPosition = Amaz.Vector3f(
        (posX / self.compositeSize.x - 0.5) * 2 * ratio,
        (1.0 - posY / self.compositeSize.y - 0.5) * 2,
    ...
    self.father_trans.localScale = Amaz.Vector3f(
        scaleX / 100.0 * (self.layerSize.x / self.compositeSize.x),
        scaleY / 100.0 * (self.layerSize.y / self.compositeSize.y),
        self.scale.z / 100.0
    )
...
function LumiLayer:onUpdate(comp, deltaTime)
    self.material:setFloat('u_opacityMix', self.opacityMix and 1 or 0)

   
    if self.yuChengFloat*self.yuChengFloat2<=0.01 then
         self.material:setFloat('u_yuCheng', 0)
    else
        self.material:setFloat('u_yuCheng',  self.yuCheng and 1 or 0)
    end

...
function LumiParamsSetter:getCurrentDefaultParam(entity, key, time)
    ...
    if keyframeName ~= nil  then
        ...
        self.paramFlags[entity .. '#' .. key] = true
    else
    ...
    for sliderKey, paramsInfos in pairs(self.sliderInfos) do
        if self.sliderParams[sliderKey] ~= nil then
            for index, value in ipairs(paramsInfos) do
                ...
                self.paramFlags[entityName .. '#' .. paramKey] = true
            end
        end
    end
end

function LumiParamsSetter:updateFadeout(lumi_obj, remainingTime)
    ...
    for i = 1, #self.fadeoutInfos.infos do
        ...
        local curValue = nil
        if self.paramFlags[entityName .. '#' .. paramKey] then
            curValue = lumi_obj:getSubEffectAttr(entityName, paramKey)
        else
            -- get original value
            local entity = self.params[entityName]
            if entity ~= nil then
                curValue = entity[paramKey]
            end
        end

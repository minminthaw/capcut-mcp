    ...
    for name, adjustKeys in pairs(makeup3dMap) do
        ...
        if makeup3dObj.entity.visible == true then
            ...
            if name == "Face3D" and meshUpdate == true then
                -- Amaz.LOGS(self.logTag, "updateMakeup3D update face mesh in index " .. faceIndex)
                local faceMVP = meshInfo.data:get("mvp")
                local faceModel = meshInfo.data:get("modelMatrix")
                local facePos = meshInfo.data:get("vertexes")
                local faceNormals = meshInfo.data:get("normals")
                -- Amaz.LOGS(self.logTag, "updateMakeup3D face pos size " .. facePos:size() .. " normals size " .. faceNormals:size())
...
function makeup:updateFaceInfoBySize()
    ...
    local facefittingCount = result:getAlgorithmInfoCount('new-model-3d-558c4d84-5927-11ed-b0cf-3e22fb85aea1', 'facefitting_3d_0', 'facefitting_3d')
    
    -- Amaz.LOGS(self.logTag, "updateFaceInfoBySize faceCount " .. faceCount .. " freidCount " .. freidCount .. " facefittingCount " .. facefittingCount)
    for i = 0, self.maxFaceNum - 1 do
        ...
        if i < faceCount then
            ...
            for j = 0, facefittingCount - 1 do
                local faceMeshInfo = result:getAlgorithmInfo('new-model-3d-558c4d84-5927-11ed-b0cf-3e22fb85aea1', 'facefitting_3d_0', 'facefitting_3d', j) 
                if (not faceMeshInfo) then
                    Amaz.LOGE(self.logTag, 'Invalid faceMeshInfo'..j)
                end
                if faceId == faceMeshInfo.data:get("face_id") then
                    meshInfo = faceMeshInfo
                end
            end

...
static inline __attribute__((always_inline))
float2 enlargeFun2(thread const float2& curCoord, thread const float2& circleCenter, thread const float& radius, thread const float& intensity)
{
    float standdis = radius * 1.2000000476837158203125;
    float infect = distance(float2(circleCenter.x, circleCenter.y), curCoord) / standdis;
    infect = 1.0 - infect;
    infect = fast::clamp(infect, 0.0, 1.0);
    float2 rescoord = float2(circleCenter.x, circleCenter.y) + ((curCoord - float2(circleCenter.x, circleCenter.y)) * (1.0 - (intensity * infect)));
    return rescoord;
}
...
vertex main0_out main0(main0_in in [[stage_in]], constant buffer_t& buffer)
{
    ...
    for (int i = 0; i < 100; i++)
    {
        ...
        else
        {
            if (abs(buffer.uDeformationActionType[i] - 1.0) < 0.001000000047497451305389404296875)
            {
                ...
                curCoord = enlargeFun2(param_5, param_6, param_7, param_8);
            }
            else
            {
                if (abs(buffer.uDeformationActionType[i] - 2.0) < 0.001000000047497451305389404296875)
                {
                    ...
                    float2 _278 = narrowFun(param_9, param_10, param_11, param_12);
                    curCoord = _278;
                }
            }
        }
    ...
    out.uv0 = in.texcoord0 + ((((curCoord / x_y) - in.texcoord0) * final_intensity) * 1.2000000476837158203125);

...
void main(void)
{
    vec4 sucaiColor = texture2D(sucaiTexture, sucaiTexCoord);
    vec2 coord = mix(origCoord, uv0, sucaiColor.r);
    // vec4 textureColor = texture2D(src_tex, coord);
    gl_FragColor = vec2ToVec4(vec4ToVec2(interp(coord-0.5/uv_W)) + coord - origCoord);
}

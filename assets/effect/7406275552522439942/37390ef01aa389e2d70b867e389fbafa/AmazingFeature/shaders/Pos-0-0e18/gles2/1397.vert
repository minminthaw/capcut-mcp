...
{
    float standdis = radius*1.2;
   
        float infect = distance(vec2(circleCenter.x,circleCenter.y), curCoord)/standdis;
        
        infect = 1.0-infect;
        infect = clamp(infect,0.0,1.0);
        
        vec2 rescoord = vec2(circleCenter.x,circleCenter.y) + (curCoord - vec2(circleCenter.x,circleCenter.y))*(1.0-intensity * infect);
...
void main()
{
    ...
    for(int i=0;i<100;i++)
    {
        ...
        {
            curCoord = enlargeFun2(curCoord, dstPoint, uDeformationRadius[i],uDeformationIntensity[i]);
        }
    ...
    textureCoordinate = inputTextureCoordinate+(curCoord/x_y-inputTextureCoordinate)*final_intensity*1.2;

...
fragment main0_out main0(main0_in in [[stage_in]], texture2d<float> u_FBOTexture [[texture(0)]], texture2d<float> sucaiTexture [[texture(1)]], sampler u_FBOTextureSmplr [[sampler(0)]], sampler sucaiTextureSmplr [[sampler(1)]])
{
    main0_out out = {};
    float4 sucaiColor = sucaiTexture.sample(sucaiTextureSmplr, in.sucaiTexCoord);
    float2 coord = mix(in.origCoord, in.uv0, float2(sucaiColor.x));
    float2 param = coord - float2(0.0009765625);
    float4 param_1 = interp(param, u_FBOTexture, u_FBOTextureSmplr);
    float2 param_2 = (vec4ToVec2(param_1) + coord) - in.origCoord;

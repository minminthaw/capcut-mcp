...
function LumiManager.new(construct, ...)
    ...
    self.event_intensity_changed = 0
...
function LumiManager:onUpdate(comp, deltaTime)
    ...
    if self.lumi_params_setter and self.lumi_params_setter.updateFadeout then
        if self.effectType == 'effect' then
            local remainingTime = self.endTime - self.curTime
            self.lumi_params_setter:updateFadeout(self.lumi_obj, remainingTime, aeTime)
        end
    end
    ...
    if self.event_intensity_changed<1 and self.lumi_params_setter then
        self.lumi_params_setter:setValue(self.lumi_obj, self.event_intensity)
        self.event_intensity_changed = self.event_intensity_changed + 1
    end
...
function LumiManager:onEvent(comp, event)
        ...
        if event.args:get(0) == 'effects_adjust_speed' then
            ...
            if self.animationInfos then
                ...
                self.speed = remap(clamp(value, 0, 1), 0, 1, self.animationInfos.speedInfo[2], self.animationInfos.speedInfo[3])
                                    Amaz.LOGE(AE_EFFECT_TAG, 'A speed: '.. self.speed .. value)
            else
                self.speed = value
                                                    Amaz.LOGE(AE_EFFECT_TAG, 'B speed: '.. self.speed .. value)
            end
        end
    end
    ...
    if key == "backlight_intensity" then
        local value = event.args:get(1)
        self.event_intensity = value
        self.event_intensity_changed = 0
    end
end

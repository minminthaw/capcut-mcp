...
uniform sampler2D u_InputTex8;
uniform float u_AngleX;
uniform float u_radius;
uniform vec4 u_ScreenParams;
uniform float u_DownSample8;
...
vec4 gaussianBlur(sampler2D i_InputTex, vec2 i_Uv, vec2 i_Dir, float i_Strength)
{
    ...
    vec2 unit_uv        = i_Dir * max(v_bloomPara.y * u_radius / 4.0, 1.0);
...
void main()
{
    ...
    float downSample = max(v_bloomPara.y * u_radius / 4.0, 1.0);
    vec4 color = gaussianBlur(u_InputTex8, uv0, dir, v_bloomPara.y * u_radius * 1.0 / downSample);
    gl_FragColor = color;
}

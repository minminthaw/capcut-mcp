...
uniform sampler2D u_InputTex3;
uniform float u_AngleX;
uniform float u_radius;
uniform vec4 u_ScreenParams;
uniform float u_DownSample3;
...
vec4 gaussianBlur(sampler2D i_InputTex, vec2 i_Uv, vec2 i_Dir, float i_Strength)
{
    ...
    vec2 unit_uv        = i_Dir * max(min(v_bloomPara.y * u_radius / 128.0, 2.0), 1.0);
...
void main()
{
    float theta = u_AngleX / 180.0 * 3.1415926;
    vec2 ratio = 720.0 * u_ScreenParams.xy / min(u_ScreenParams.x, u_ScreenParams.y);
    vec2 dir = vec2(cos(theta), sin(theta)) / ratio.xy;
    float downSample = max(min(v_bloomPara.y * u_radius / 128.0, 2.0), 1.0);
    vec4 color = gaussianBlur(u_InputTex3, uv0, dir, v_bloomPara.y * u_radius / 16. * 0.6 / downSample);
    gl_FragColor = color;
}

...
vec4 gaussianBlur(sampler2D i_InputTex, vec2 i_Uv, vec2 i_Dir, float i_Strength)
{
    ...
    vec2 unit_uv        = i_Dir * max(v_bloomPara.y * u_radius / 8.0, 1.0);
...
void main()
{
    ...
    float downSample = max(v_bloomPara.y * u_radius / 8.0, 1.0);
    vec4 color = gaussianBlur(u_InputTex7, uv0, dir, v_bloomPara.y * u_radius * 0.5 / downSample);
    gl_FragColor = color;
}

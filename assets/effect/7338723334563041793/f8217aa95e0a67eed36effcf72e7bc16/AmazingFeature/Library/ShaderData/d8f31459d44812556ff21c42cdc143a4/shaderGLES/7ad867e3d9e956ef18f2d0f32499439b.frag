...
float colorBurn(float s, float d)
{
    ...
    else return 1.0 - min(1.0, (1.0 - d) / s);
}
...
void main()
{
    vec4 baseColor = texture2D(u_BaseTexure, uv0);
    if (u_NeedBlend == 0)
    {
    ...
    vec4 srcColor = texture2D(u_SourceTexture, uv0);
    ...
    if (u_BlendMode == 1)
    {
        result.rgb = BlendAdd(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 2)
    {
        result.rgb = BlendMultiply(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 3)
    {
        result.rgb = BlendDifference(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 4)
    {
        result.rgb = BlendOverlay(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 5)
    {
        result.rgb = BlendDarken(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 6)
    {
        result.rgb = BlendLighten(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 7)
    {
        result.rgb = BlendSoftLight(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 8)
    {
        result.rgb = BlendHardLight(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 9)
    {
        result.rgb = BlendHue(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 10)
    {
        result.rgb = BlendSaturation(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 11)
    {
        result.rgb = BlendColor(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 12)
    {
        result.rgb = BlendScreen(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 13)
    {
        result.rgb = BlendColorBurn(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 14)
    {
        result.rgb = BlendLinearBurn(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 15)
    {
        result.rgb = BlendColorDodge(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 16)
    {
        result.rgb = BlendLinearDodge(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 17)
    {
        result.rgb = BlendVividLight(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 18)
    {
        result.rgb = BlendLinearLight(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 19)
    {
        result.rgb = BlendPinLight(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 20)
    {
        result.rgb = BlendHardMix(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 21)
    {
        result.rgb = BlendExclusion(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 22)
    {
        result.rgb = BlendSubtract(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 23)
    {
        result.rgb = BlendDivide(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 24)
    {
        result.rgb = BlendLuminosity(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 25)
    {
        result.rgb = BlendLighterColor(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 26)
    {
        result.rgb = BlendDarkerColor(srcColor.rgb, baseColor.rgb);
    }
    else if (u_BlendMode == 27) // Dissolve
    {
        if (srcColor.a == 1.0 || (srcColor.a > 0.0 && srcColor.a > random(uv0)))
        {
            result.rgb = srcColor.rgb;
        }
    }
    else // normal
    {
    ...
    // The following code is to perform "alpha compositing". Please NOTICE that: 
    // the input "srcColor" and "baseColor" have straight alpha, while the "result" 
    // has premultiplied alpha (because result.rgb does NOT divided by result.a)..
    result.rgb = result.rgb * srcColor.a + baseColor.rgb * baseColor.a * (1.0 - srcColor.a);

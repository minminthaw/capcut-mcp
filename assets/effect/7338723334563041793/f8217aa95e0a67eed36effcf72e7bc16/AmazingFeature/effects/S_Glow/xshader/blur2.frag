...
vec4 neighborColor(float i)
{
    ...
    return vec4(0.0,0.0,vec4ToVec2(texture2D(inputTexture, uv1))) * s.x 
            + vec4(0.0,0.0,vec4ToVec2(texture2D(inputTexture, uv2))) * s.y;
#endif
...
void main()
{
    ...
    curColor.ba = vec4ToVec2(texture2D(inputTexture, uv0));
#endif
    gl_FragColor = vec2ToVec4(blurColor(curColor).ba);
}

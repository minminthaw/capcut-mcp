    ...
    for (int i = 0; i < n; i++)
    {
        ...
        else
        {
            if (abs(buffer.uDeformationActionType[i] - 1.0) < 0.001000000047497451305389404296875)
            {
                ...
                float2 _237 = enlargeFun(param_5, param_6, param_7, param_8);
                curCoord = _237;
            }
            else
            {
                if (abs(buffer.uDeformationActionType[i] - 2.0) < 0.001000000047497451305389404296875)
                {
                    ...
                    float2 _260 = narrowFun(param_9, param_10, param_11, param_12);
                    curCoord = _260;
                }
            }
        }
    }
    float final_intensity = (2.0 * (0.5 - buffer.uIntensity)) * step(buffer.uIntensity, 0.5);
    float gradual = 1.0 + ((0.5 * (final_intensity - 0.800000011920928955078125)) * step(0.800000011920928955078125, final_intensity));
    final_intensity *= gradual;
    out.uv0 = in.texcoord0 + (((curCoord / x_y) - in.texcoord0) * final_intensity);
    out.maskCoord = in.texcoord1;

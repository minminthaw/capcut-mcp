...
float samplingGaussian(sampler2D tex, vec2 uv, float sigma) {
    ...
    for (int i = 1; i <= sample; i++) {
        ...
        dis = offset.y * offset.y;
        gaussWeight = gaussianSimplfy(dis, sigma);
        alpha += texture2D(tex, uv + vec2(0.0, offset.y)).r * gaussWeight;
...
void main()
{
    ...
    //     {
    //         alpha = samplingGaussian(u_maskTex, transformedUV, u_diff);
    //     }
    // }
    // else
    // {
    ...
    vec4 mask = vec4(alpha, 0.0, 0.0, 1.0);
    gl_FragColor = vec4(addBlend(color.rgb, mask.rgb), mask.a);
    // gl_FragColor = texture2D(u_maskTex, uv0); 
}
...
float4 _f6(thread float4& _p0, thread float4& _p1, constant int& u_enableMatte, texture2d<float> u_maskTexture, sampler u_maskTextureSmplr, thread float2& uv0, constant int& u_matteMode, constant int& u_blendMode, constant int& u_layerType, constant int& u_hasMatte, constant float& u_layerOpacity)
{
    ...
    if (u_blendMode == 12)
    {
        float3 _528 = (_p0.xyz + _p1.xyz) - (_p0.xyz * _p1.xyz);
        _t33.x = _528.x;
        _t33.y = _528.y;
        _t33.z = _528.z;
    }
    ...
    if (u_layerType == 1)
    {
        ...
        float4 _575 = mix(_p1, float4(_t33.xyz, _p0.w), float4(u_layerOpacity * _t35));
        _t34 = _575;
        float _577 = _t34.w;
        float3 _580 = _575.xyz * _577;
        _t34.x = _580.x;
        _t34.y = _580.y;
        _t34.z = _580.z;
    }
    ...
    {
        float3 _615 = (((_p1.xyz * _p1.w) * (1.0 - _p0.w)) + ((_p0.xyz * _p0.w) * (1.0 - _p1.w))) + (_t33.xyz * (_p0.w * _p1.w));
        _t34.x = _615.x;
        _t34.y = _615.y;
        _t34.z = _615.z;
...
fragment main0_out main0(main0_in in [[stage_in]], constant buffer_t& buffer, texture2d<float> u_maskTexture [[texture(0)]], texture2d<float> u_baseTexure [[texture(1)]], texture2d<float> u_sourceTexture [[texture(2)]], sampler u_maskTextureSmplr [[sampler(0)]], sampler u_baseTexureSmplr [[sampler(1)]], sampler u_sourceTextureSmplr [[sampler(2)]])
{
    ...
    if (buffer.u_hasTrs == 1)
    {
        ...
        float2 _676 = _f2(param, param_1, param_2);
        float _680 = step(buffer.u_mirrorEdge, 0.5);
        float2 param_3 = _676;
        float2 _690 = (_676 * _680) + (_f4(param_3) * (1.0 - _680));
        float2 param_4 = _690;
        _t37 = (u_sourceTexture.sample(u_sourceTextureSmplr, _690) * buffer.u_alpha) * _f3(param_4);
    }
    else
    {
    ...
    if (_638)
    {
        float4 param_6 = _t37;
        float4 param_7 = _t36;
        float4 _728 = _f6(param_6, param_7, buffer.u_enableMatte, u_maskTexture, u_maskTextureSmplr, in.uv0, buffer.u_matteMode, buffer.u_blendMode, buffer.u_layerType, buffer.u_hasMatte, buffer.u_layerOpacity);
        _t37 = _728;
    }

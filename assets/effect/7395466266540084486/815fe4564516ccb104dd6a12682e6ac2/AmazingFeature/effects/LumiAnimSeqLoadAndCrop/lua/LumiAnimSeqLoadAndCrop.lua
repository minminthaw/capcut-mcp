...
setmetatable(cropTypeIndex, {
    __index = function(_, key)
        Amaz.LOGE("AE_LUA_TAG", "Unsupported SeqCrop Type: " .. key)
        return 0
    end
})
...
setmetatable(edgeTypeIndex, {
    __index = function(_, key)
        Amaz.LOGE("AE_LUA_TAG", "Unsupported Edge Type: " .. key)
        return 0
    end
})
...
function LumiAnimSeqLoadAndCrop:onStart(comp)
    self.camera = comp.entity:searchEntity("AnimSeqCamera"):getComponent("Camera")
    self.renderer = comp.entity:searchEntity("AnimSeqPass"):getComponent("Sprite2DRenderer")
    self.material = self.renderer.material
    self.videoAnim = comp.entity:searchEntity("AnimSeqPass"):getComponent("VideoAnimSeq")
    self.seqAnim = comp.entity:searchEntity("AnimSeqPass"):getComponent("AnimSeqComponent")
end
...
function LumiAnimSeqLoadAndCrop:setEffectAttr(key, value, comp)
    ...
    elseif key == "seqTex" then
        _setEffectAttr("seqTexName", value, comp)
...
function LumiAnimSeqLoadAndCrop:onUpdate(comp, deltaTime)
    ...
    if self.animSeqType == "Video" then
        ...
        self.videoAnim:seekToTime(self.seqTime)
    ...
    elseif self.animSeqType == "Image" then
        ...
        self.seqAnim:seekToTime(self.seqTime)
    elseif self.animSeqType == "Texture" then
        self.videoAnim.enabled = false
        self.seqAnim.enabled = false
        self.renderer.flip = false

        if self.seqTexName then
            local seqTex = Amaz.BuiltinObject.getUserTexture(self.seqTexName)
            if seqTex then
                self.seqTex = seqTex
            end
        end

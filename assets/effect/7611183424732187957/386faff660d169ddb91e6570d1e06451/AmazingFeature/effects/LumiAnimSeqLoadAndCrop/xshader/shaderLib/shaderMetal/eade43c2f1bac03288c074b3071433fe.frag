...
{
    float2 _122 = mod(_p0, float2(1.0));
    return _122 + (float2(1.0) - step(float2(0.0), _122));
}
...
float4 _f0(thread float2& _p0, constant int& u_yFlip, constant int& u_convertAlpha, texture2d<float> u_seqTexture, sampler u_seqTextureSmplr)
{
    ...
    float3 _97 = u_seqTexture.sample(u_seqTextureSmplr, float2((_p0.x * 0.5) + 0.5, _p0.y)).xyz;
    _t2.x = _97.x;
    _t2.y = _97.y;
    _t2.z = _97.z;
    return _t2;
...
{
    main0_out out = {};
    bool _214 = abs(buffer.u_scale.x) < 9.9999997473787516355514526367188e-05;
    bool _222;
    if (!_214)
    {
        _222 = abs(buffer.u_scale.y) < 9.9999997473787516355514526367188e-05;
    }
    else
    {
        _222 = _214;
    ...
    float2 _245 = _f4(param, param_1, param_2, param_3, param_4, buffer.u_ScreenParams);
    float2 _t8 = _245;
    float _252 = buffer.u_seqTexSize.x / buffer.u_seqTexSize.y;
    float _258 = buffer.u_ScreenParams.x / buffer.u_ScreenParams.y;
    float2 _t11 = _245;
    ...
    float2 param_7 = _t11;
    float4 _389 = _f0(param_7, buffer.u_yFlip, buffer.u_convertAlpha, u_seqTexture, u_seqTextureSmplr);
    float4 _t16 = _389;
    ...
    float4 _401 = _t16;
    float4 _402 = _401 * buffer.u_opacity;
    _t16 = _402;
    out.o_fragColor = _402;
    return out;
}

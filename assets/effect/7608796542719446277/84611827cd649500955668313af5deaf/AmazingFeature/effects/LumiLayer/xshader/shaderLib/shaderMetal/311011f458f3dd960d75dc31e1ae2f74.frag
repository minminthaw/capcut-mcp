...
static inline __attribute__((always_inline))
float4 _f6(thread float4& _p0, thread float4& _p1, constant int& u_enableMatte, texture2d<float> u_maskTexture, sampler u_maskTextureSmplr, thread float2& uv0, constant int& u_matteMode, constant int& u_blendMode, constant int& u_layerType, constant int& u_hasMatte, constant float& u_layerOpacity)
{
    float _483 = _p0.w;
    float4 _486 = _p0;
    ...
    if (u_blendMode == 4)
    {
        ...
        if (_p1.x < 0.5)
        {
            _522 = (2.0 * _p1.x) * _p0.x;
        }
        else
        {
            _522 = 1.0 - ((2.0 * (1.0 - _p1.x)) * (1.0 - _p0.x));
        }
        float _545;
        if (_p1.y < 0.5)
        {
            _545 = (2.0 * _p1.y) * _p0.y;
        }
        else
        {
            _545 = 1.0 - ((2.0 * (1.0 - _p1.y)) * (1.0 - _p0.y));
        }
        float _568;
        if (_p1.z < 0.5)
        {
            _568 = (2.0 * _p1.z) * _p0.z;
        }
        else
        {
            _568 = 1.0 - ((2.0 * (1.0 - _p1.z)) * (1.0 - _p0.z));
        }
        float3 _588 = float3(_522, _545, _568);
        _t33.x = _588.x;
        _t33.y = _588.y;
        _t33.z = _588.z;
    }
    ...
    if (u_layerType == 1)
    {
        ...
        float4 _635 = mix(_p1, float4(_t33.xyz, _p0.w), float4(u_layerOpacity * _t35));
        _t34 = _635;
        float _637 = _t34.w;
        float3 _640 = _635.xyz * _637;
        _t34.x = _640.x;
        _t34.y = _640.y;
        _t34.z = _640.z;
    }
    ...
    {
        float3 _675 = (((_p1.xyz * _p1.w) * (1.0 - _p0.w)) + ((_p0.xyz * _p0.w) * (1.0 - _p1.w))) + (_t33.xyz * (_p0.w * _p1.w));
        _t34.x = _675.x;
        _t34.y = _675.y;
        _t34.z = _675.z;
...
fragment main0_out main0(main0_in in [[stage_in]], constant buffer_t& buffer, texture2d<float> u_maskTexture [[texture(0)]], texture2d<float> u_baseTexure [[texture(1)]], texture2d<float> u_sourceTexture [[texture(2)]], sampler u_maskTextureSmplr [[sampler(0)]], sampler u_baseTexureSmplr [[sampler(1)]], sampler u_sourceTextureSmplr [[sampler(2)]])
{
    ...
    if (buffer.u_hasTrs == 1)
    {
        ...
        float2 _736 = _f2(param, param_1, param_2);
        float _739 = step(buffer.u_mirrorEdge, 0.5);
        float2 param_3 = _736;
        float2 _749 = (_736 * _739) + (_f4(param_3) * (1.0 - _739));
        float2 param_4 = _749;
        _t37 = (u_sourceTexture.sample(u_sourceTextureSmplr, _749) * buffer.u_alpha) * _f3(param_4);
    }
    else
    {
    ...
    if (_698)
    {
        float4 param_6 = _t37;
        float4 param_7 = _t36;
        float4 _787 = _f6(param_6, param_7, buffer.u_enableMatte, u_maskTexture, u_maskTextureSmplr, in.uv0, buffer.u_matteMode, buffer.u_blendMode, buffer.u_layerType, buffer.u_hasMatte, buffer.u_layerOpacity);
        _t37 = _787;
    }

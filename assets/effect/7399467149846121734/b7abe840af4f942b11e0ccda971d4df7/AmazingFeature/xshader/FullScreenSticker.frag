...
#define BLEND_MODE blendNormal      //cccccc        
...
{
    vec3 resultColor = BLEND(base,blend,BLEND_MODE);   //cccccc  
...
{
    vec4 resultColor = vec4(clamp(color.rgb/color.a,0.0,1.0),color.a);
    return resultColor;
}

void main(void) 
{
    vec2 sucaiUV = vec2(uv0.x,1.0-uv0.y);   //amazing-engineccccflip
    vec4 fgColor = texture2D(_MainTex, sucaiUV);    //sucai
    fgColor = lm_inverse_premult(fgColor);
    // fgColor = fgColor * alphaFactor;
        
    vec4 bgColor = texture2D(inputImageTexture, uv0);
    vec4 resultColor = vec4(blendFunc(bgColor.rgb,fgColor.rgb,fgColor.a),1.0);

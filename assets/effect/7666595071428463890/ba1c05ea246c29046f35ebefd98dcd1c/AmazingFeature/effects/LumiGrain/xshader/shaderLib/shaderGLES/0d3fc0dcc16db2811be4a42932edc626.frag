...
uniform mediump int u_monochrome;
...
uniform mediump int u_onlyNoise;
...
void main()
{
    ...
    _t21 = clamp(_f0(param_18, param_19).xyz, vec3(0.0), vec3(1.0));
    if (u_onlyNoise == 1)
    {
        gl_FragData[0] = vec4(_t21, 1.0);
        return;
    }
    mediump vec4 _598 = texture2D(u_InputTexture, v_uv);
    vec4 _t22 = _598;
    vec4 _t23;
    if (_525)
    {
        _t23 = vec4(_t21 + _598.xyz, _t22.w);
    }
    else
    {
        if (_528)
        {
            _t23 = vec4(_598.xyz * _t21, _t22.w);
        }
        else
        {
            if (u_combine == 3)
            {
                ...
                _t23 = vec4(vec3(_638, _661, _684), _t22.w);
            }
            else
            {
                if (u_combine == 4)
                {
                    ...
                    vec3 _823 = vec3(_757, _780, _803);
                    _t23 = vec4(mix(_823, vec3(1.0) - ((vec3(1.0) - _823) * (vec3(1.0) - vec3(max(1.0 - ((1.0 - _t21.x) / 0.5), 0.0), max(1.0 - ((1.0 - _t21.y) / 0.5), 0.0), max(1.0 - ((1.0 - _t21.z) / 0.5), 0.0)))), vec3(0.5 * smoothstep(0.588235318660736083984375, 0.509803950786590576171875, dot(_598.xyz, vec3(0.2989999949932098388671875, 0.58700001239776611328125, 0.114000000059604644775390625))))), _t22.w);
                }
                else
                {
                    if (u_combine == 5)
                    {
                        _t23 = vec4(abs(_t21 - _598.xyz), _t22.w);
                    }
                    else
                    {
                        _t23 = vec4(vec3(1.0) - ((vec3(1.0) - _598.xyz) * (vec3(1.0) - _t21)), _t22.w);
                    }
                }
            }
        }

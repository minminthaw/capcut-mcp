...
-- input
local BODY_ID = "id"
local BODY_ADJUST_INTENSITY = "natural_body_intensity"
...
local SlimBodyMap = {
    ...
    SlimArm = {
        ...
        min_ratio = 0.0,
        --- ori 1.0, 正向 40, 1.0 * 0.4
        max_ratio = 0.4
    },
    LongLeg = {
        type = Amaz.NsItemType.STRETCH_LEG,
        min_ratio = 0.0,
        --- ori 0.35, 正向 10, 0.35 * 0.1
        max_ratio = 0.035
    },
    SlimBody = {
        type = Amaz.NsItemType.SLIM_BODY,
        min_ratio = 0.0,
        --- ori 0.3, 正向 30, 0.3 * 0.3
        max_ratio = 0.09
    },
}
...
function slimbody:onEvent(comp, event)
    printSlimbodyLog(self.logTag .. " [cxy debug] slimbody:onEvent")
...
function slimbody:updateItems(comp, deltaTime)
    ...
    printSlimbodyLog(self.logTag .. " [zhw debug] updateItems enter")
    local items = self.slimbodyComp.items
    local itemsCount = items:size()
    for key, info in pairs(SlimBodyMap) do
        local itemType = info.type
        local itemIntensity = self.intensityMap[key]
        printSlimbodyLog(self.logTag .. " [cxy debug] updateItems enter itemIntensity: " .. tostring(itemIntensity))
        for i = 0, itemsCount - 1 do
            local item = items:get(i)
